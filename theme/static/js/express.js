...
(() => {
  ...
  let challenges = [], folder = 'All Challenges', selected = null, generation = 0;
  ...
  function render() {
    const categories = ['All Challenges', ...new Set(challenges.map(c => c.category))];
    $('#folders').innerHTML = categories.map(category => `<button type="button" class="${folder === category ? 'active' : ''}" data-folder="${escape(category)}"><i class="fas fa-folder" aria-hidden="true"></i>${escape(category)} (${challenges.filter(c => !c.solved_by_me && (category === 'All Challenges' || c.category === category)).length})</button>`).join('');
    $('#progress').textContent = `${challenges.filter(c => c.solved_by_me).length} of ${challenges.length} challenges solved`;
    $('#folder-heading').textContent = $('#address').textContent = folder;
    document.title = $('#window-title').textContent = `${window.init.appName} - ${folder}`;
    const query = $('#search').value.toLowerCase().trim();
    const visible = challenges.filter(c => (folder === 'All Challenges' || c.category === folder) && `${c.name} ${c.category}`.toLowerCase().includes(query));
  ...
  async function open(id) {
    ...
    try {
      ...
      const view = new DOMParser().parseFromString(c.view, 'text/html');
      c.description = view.querySelector('.challenge-desc')?.innerHTML || escape(c.description);
  ...
  $('#folders').addEventListener('click', event => { const button = event.target.closest('[data-folder]'); if (button) { folder = button.dataset.folder; back(); render(); } });
  $('#all-challenges').addEventListener('click', () => { folder = 'All Challenges'; $('#search').value = ''; back(); render(); });
